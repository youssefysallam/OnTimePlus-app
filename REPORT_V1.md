...
UMass Boston students commute via the MBTA Red Line (or transfers from the
Orange / Blue / Green / Bus 8 / Quincy branch) and a campus shuttle that
runs JFK/UMass ↔ Campus Center every 5–10 minutes on weekdays. Common
questions students ask are:

- *"I have a 9 AM class at UMass Boston, leaving from JFK/UMass — when
  should I leave?"*
- *"It's snowing today. My class is at 11 AM and I'm coming from
  Alewife — what's the safest plan?"*
- *"The Red Line has a signal problem near Park Street and my midterm
  is at 9 AM — what should I do?"*

Public MBTA tools (the official app, Google Maps) give a point estimate
of travel time but do **not** explain whether the user has *enough
buffer* given **current alerts, weather, and the stakes of the event**.
OnTime+ v1 fills that gap with a Retrieval-Augmented Generation (RAG)
pipeline that produces a recommended departure time **plus** an explicit
risk label (`reliable` / `caution` / `risky`) and grounded citations to
...
and writes to the shared `GraphState`, so each step can be inspected
independently from the FastAPI / Streamlit / future-mobile clients.

### 2.2 Why split LLM and rule-based steps?
...
set** (used for the metrics in §6), and a small **conversational
...
| `alert`            | `data/simulated/disruption_scenarios.jsonl` (6) + `data/simulated/weather_scenarios.jsonl` (7) | 13 | Simulated MBTA / UMB alerts   | Each carries `severity ∈ {low, moderate, high}`, `added_min`, an activation policy (see §4.4) and `trigger_keywords` |
...
high-stakes flag, weather type, and disruption type, and is small but
hand-labelled enough to drive metric-grounded iteration.

### 3.3 Schedule context (optional)
...
3. Walk the retrieved alert documents with the **alert activation
   filter** (§4.4); for each active alert that overlaps the planned
...
### 4.4 Alert activation filter (`harness/alert_filter.py`)
...
#### 4.7.2 Node 1 prompts (intent extraction)
...
#### 4.7.3 Node 5 prompts (answer composition)
...
#### 4.7.4 What we explicitly chose *not* to put in the prompts
...
- **No "infer a buffer" instruction.** The LLM is never asked to
  ...
  job (§4.5). The LLM only echoes the precomputed label.
...
The 20 hand-labelled queries in `eval/test_queries_v2.jsonl` (§3.2),
balanced across four themes and the three risk classes (`reliable`
n = 9, `caution` n = 4, `risky` n = 7).

### 5.2 Metric definitions

We report three metric families, mirroring the three structured
...
| Metric | Definition |
|---|---|
| **Exact match (EM)** | Fraction of queries where **all five** intent slots match the gold (case-insensitive string match for strings, equality for booleans). |
| **Per-slot accuracy** | Fraction of queries where slot $s$ matches the gold. Computed for $s \in \{\text{origin}, \text{destination}, \text{deadline}, \text{depart\_time}, \text{high\_stakes}\}$. |

#### (b) Retrieval

For every query, the gold relevant document set $G_q \subseteq \mathcal{D}$
is given, and the system returns a ranked list $R_q = (d_1, d_2, \ldots)$.

| Metric | Definition |
|---|---|
| **Recall@k** | $\frac{1}{N} \sum_{q} \frac{|R_q^{(k)} \cap G_q|}{|G_q|}$ where $R_q^{(k)}$ is the top-$k$ prefix of $R_q$ |
| **Precision@k** | $\frac{1}{N} \sum_{q} \frac{|R_q^{(k)} \cap G_q|}{k}$ |
| **MRR** | $\frac{1}{N} \sum_{q} \frac{1}{\min_{g \in G_q} \text{rank}_q(g)}$ — reciprocal of the rank of the **first relevant** doc; 0 if none retrieved. |

We report $k \in \{3, 5, 10\}$.

#### (c) Risk labelling

Three-class classification (`reliable` / `caution` / `risky`).

| Metric | Definition |
|---|---|
| **Accuracy** | Fraction of queries where predicted label = gold label. |
| **Macro-F1** | $\frac{1}{3} \sum_{c \in \{\text{reliable}, \text{caution}, \text{risky}\}} F_1(c)$ where $F_1(c) = 2 \cdot \frac{P_c \cdot R_c}{P_c + R_c}$, computed via `sklearn.metrics.f1_score(..., average="macro", zero_division=0)`. |
| **Per-class F1** | $F_1(c)$ for each class individually. Useful because the classes are imbalanced. |
| **Confusion matrix** | $C_{g,p} = |\{q : \text{gold}_q = g, \text{pred}_q = p\}|$ for $g, p \in \{\text{reliable}, \text{caution}, \text{risky}\}$. |
| **Support** | Number of gold instances per class. |
...
- `eval_predictions_v3.jsonl` — one record per query (intent, ranking,
  predicted label, suggested depart, full answer).
- `eval_metrics_v3.json` — aggregate numbers (overall + per-theme).
- `EVALUATION_REPORT_v3.md` — auto-generated summary table.

(The `_v3` suffix is just the on-disk filename used by the evaluation
runner — these are the v1 numbers reported below.)
...
All 9 `reliable` cases are predicted correctly (no false alarms in
the easy bucket), and no `risky` case is downgraded all the way to
`reliable`. The two remaining errors are one `caution → reliable`
(q03) and one `risky → caution` (q12) — both because the relevant
alert document was not pulled in despite the targeted alert pass.
See §7 for the planned fix.

### 6.2 Per-theme breakdown

| Theme       | n | Intent EM | MRR | Recall@5 | Risk Accuracy | Risk Macro-F1 |
|---|---:|---:|---:|---:|---:|---:|
| `normal`    | 5 | **100.0 %** | 0.900 | 63.3 % | **100.0 %** | 0.667 |
| `time`      | 5 | 80.0 %    | 0.800 | 60.0 % | **100.0 %** | 0.333 (only `reliable` in support) |
| `weather`   | 5 | 80.0 %    | 0.840 | 50.0 % | 80.0 %      | 0.556 |
| `disruption`| 5 | 80.0 %    | 0.867 | 50.0 % | 80.0 %      | 0.508 |

(Macro-F1 in `time` is mechanically low because the gold support
contains only `reliable` — there are no `caution` / `risky` instances
to compute F1 on, and `sklearn` returns 0 for those classes.)

### 6.3 Case studies

This section walks through seven concrete queries from the evaluation
set: five **successful** cases that exercise distinct design choices in
v1, and the two **failure** cases that account for the entire 2-error
gap from a perfect risk score. Each walkthrough shows the full pipeline
trace (intent → retrieved evidence → route → risk → answer) so the
behavior is fully reproducible from `eval_predictions_v3.jsonl`.

#### 6.3.1 Successful cases

##### Case A — `q01` (theme: `normal`) · the vanilla case
...
**What this case demonstrates.** The two-mode buffer policy (§4.5)
correctly returns `reliable` for short, alert-free trips. Under the
old single-mode rule, a 7-minute built-in buffer (`σ·std ≈ 7 min`)
would have been below the 10-minute reliable threshold and the
answer would have been mislabeled `caution`. Here the system
correctly identifies an "alert-free + low-stakes + safe-margin"
profile as reliable.

##### Case B — `q04` (theme: `disruption`, high-stakes) · alert + high-stakes escalation
...
**What this case demonstrates.** Three v1 features all fire together:
(1) the few-shot prompt picks up `high_stakes=True` from "midterm",
(2) the targeted alert pass guarantees the signal alert lands in
evidence, and (3) the alert-activation override lets a `time`-mode
alert fire when its keyword is in the query, even if the query time
is outside the nominal `07:30–10:00` window. Without any one of these
three, this query would have been labeled `caution` or worse.

##### Case C — `q07` (theme: `weather`) · severe weather
...
**What this case demonstrates.** The KB synonym expansion (§3.1)
makes the snow alert retrievable via natural-language phrasing
("snowstorm") rather than just the literal word "snow". The risk
analyzer correctly treats a single high-severity weather alert as
sufficient grounds for `risky`, even without a high-stakes flag.

##### Case D — `q08` (theme: `disruption`, user-chosen depart) · two-mode buffer (literal mode)
...
| Stage | Output |
|---|---|
| **Intent (pred)** | `origin=JFK`, `destination=UMass Boston`, `deadline=10:00`, `depart_time=09:50`, `high_stakes=True` (few-shot picked up "finals" — see note in §7) |
...
**What this case demonstrates.** This is the *literal* branch of the
two-mode buffer policy: the user pinned the departure, so the system
must report whether *that* plan fits, not invent a safer one. The
correct behavior is to surface the negative buffer (–20 min) and
label `risky`. The same query handled by the system-suggested branch
would have produced a depart-time recommendation around `09:30`
instead.

##### Case E — `q15` (theme: `normal`, high-stakes) · few-shot for high-stakes intent
...
**What this case demonstrates.** A common failure mode would be
labeling a high-stakes trip `caution` simply because the user said
"interview" — but here the *plan* is genuinely safe (no alerts, ample
margin), so the correct answer is `reliable`. v1 distinguishes
between **"the user is anxious"** (which raises σ and shifts the
suggested depart earlier) and **"the trip is risky"** (which requires
real evidence — alerts, negative buffer, or both). q15 confirms the
former does not collapse into the latter.

#### 6.3.2 Failure cases

##### Case F — `q03` (theme: `weather`) · gold = `caution`, pred = `reliable` · *retrieval miss*
...
| **Route plan** | South Station → JFK/UMass → Campus Center, `total ≈ 15 min`, `alert_added = 0` (the rain alert is missing, no other active alert applies to South Station → UMB) |
...
fused list, even after the targeted alert pass, because two other
weather alerts (`alert-red-snow`, `alert-shuttle-wind`) outranked it
under both BM25 and BGE. Once the alert is missing from evidence,
every downstream stage (route, risk, answer) is correct *given* the
evidence, but the evidence itself is incomplete.

**Planned fix.** A second-stage cross-encoder reranker over `top-30`
fused candidates would resolve this by promoting `alert-red-rain`
(which shares the literal keyword "rain" with the query) above the
two unrelated weather alerts. This is item #1 in §7.

##### Case G — `q12` (theme: `disruption`, double-alert) · gold = `risky`, pred = `caution` · *partial alert recall*
...
| **Answer (first line)** | "I recommend departing from Alewife by 9:20 AM to make your 10 AM class at UMass Boston. With the Red Line delayed and the UMass Boston shuttle running reduced, the total expected travel time is about 40 minutes…" |

**Root cause.** Partial alert recall — one of the two simultaneously
active alerts (`alert-red-signal-2026-04-26`) was not retrieved.
Note that the *natural-language description* in the answer correctly
mentions "Red Line delayed" because the LLM read it from the query,
but the **decision pipeline** did not have a structured alert
document to attribute the +12 min to, so the cumulative alert delay
fell below the `risky` threshold. The targeted alert pass with
keyword "delayed" did fire but `alert-red-signal-2026-04-26` lost
to `alert-shuttle-reduced` (which exactly matches the query verbatim).

**Planned fix.** Same as case F — a cross-encoder reranker over the
combined RRF + targeted-alert candidate pool. A second, lower-cost
fix is to add `red line is delayed`, `delays on the red line` to the
`trigger_keywords` of `alert-red-signal-*` and pass *all* alerts
matching any query keyword (not just the top-K), bypassing the
similarity ranking step for keyword-matched alerts entirely. This is
also item #1 in §7.
